...
var classes_suggestjs = {
	...
	// alert(val);
	 ...
	 $.getJSON(wl_tmp+'classes_search.php?q='+val+'&start=0&length=10&in=json&callback=?', 
		 ...
		 function (data) {
			var classessearch = '<table cellspacing="0" border="0" cellpadding="5" class="altrowstable" width="100%" id="classes_row"><tr>';
			var tr_i=0;
			
			$.each(data.data, function (i, item) {
				
				if(tr_i % 2 == 0){
					tr_class = "evenrowcolor";	
				}else{
					tr_class = "oddrowcolor";
				}
	...
	set_inputstring: function (str){
		...
		//alert(getAttributeByName($_('c'+(classes_suggestjs.row-1)), "suggestcid"));
		if(str == 'submit'){
